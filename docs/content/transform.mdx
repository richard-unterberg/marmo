...
The selected element's props are inferred automatically. `$` props from the source Marmo component are preserved, while ordinary props are validated against the transformed intrinsic element.

```tsx choice="react"
const Example = () => (
  <>
    <Text $_as="label" htmlFor="email">Email</Text>
    <Text $_as="span" className="text-sm">Inline information</Text>
  </>
)
```
...
const Example = () => (
  <>
    <Text $_as="label" for="email">Email</Text>
    <Text $_as="span" class="text-sm">Inline information</Text>
  </>
)
```
...
---
### v1 limitation: Marmo components only
...
```jsx choice="solidjs"
<Alert class={inline() ? "inline" : "block"} />
```
