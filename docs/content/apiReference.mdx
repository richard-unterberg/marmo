...
React imports come from `@marmo/react`; Solid imports come from `@marmo/solid`. Both packages intentionally expose the same core factory and utilities, with one framework-specific component-local helper.
